...
export class Game {
	...
	playSkillAudio(name, index) {
		...
		const candidates = getAudioSourceCandidates(`${str}${name}`)
			.concat(`${str}${name}.ogg`)
			.concat(getAudioSourceCandidates(`${str}${name}${index}`))
			.concat(`${str}${name}${index}.ogg`);
		setAudioSourceWithFallback(
			audio,
			candidates.map(current => `${lib.assetURL}${current}`),
			() => audio.remove(),
		);
		ui.window.appendChild(audio);
	}
	...
	playBackgroundMusic() {
		...
		if (_status._aozhan) {
			...
			if (["blob:", "data:"].some(prefix => aozhan.startsWith(prefix))) {
				...
				game.getDB("image", aozhan.slice(3)).then(result => setSource(result));
		...
		if (["blob:", "data:"].some(prefix => music.startsWith(prefix))) {
			...
			game.getDB("image", music.slice(3)).then(result => setSource(result));
