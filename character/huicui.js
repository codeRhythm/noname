...
const skills = {
  ...
  chijian: {
    ...
    locked: false,
    init(player, skill) {
      player.addExtraEquip(skill, "qinggang", true, (player2) => player2.hasEmptySlot(1) && !player2.getVEquip(1) && lib.card.qinggang);
    },
    onremove(player, skill) {
  ...
  shiwu: {
    ...
    async content(event, trigger, player) {
      ...
      if (damagedEvents.some((evt) => evt.source === target && evt.player === player)) {
        ...
        player.when({ global: "phaseEnd" }).filter((evt) => trigger.getParent("phase", true, true) === evt).step(async (event2, trigger2, player2) => {
